...
let compareTime = (str1, str2) => {
    let output;
    let time1 = getIntTime(str1);
    let time2 = getIntTime(str2);
    if (time1 < time2)
        output = -1;
    else
        output = 1;
...
let mergeTimes = (c1, c2) => {
    let output = [];
    let pointer1 = 0;
    let pointer2 = 0;
    while (pointer1 < c1.blockedTimes.length || pointer2 < c2.blockedTimes.length) {
        let tb1 = c1.blockedTimes[pointer1];
        let tb2 = c2.blockedTimes[pointer2];
...
let addBounds = (c1, c2) => {
    ...
    if (compareTime(start1, start2) == -1)
        startArr = [start2, start2];
    else
        startArr = [start1, start1];
    if (compareTime(end1, end2) == -1)
        endArr = [end1, end1];
    else
        endArr = [end2, end2];
...
let flattenCalendar = (c1, c2) => {
    let output = addBounds(c1, c2);
    for (let i = 1; i < output.length; i++) {
        if (compareTime(output[i - 1][0], output[i][0]) == 1)
            output[i][0] = output[i - 1][0];
        if (compareTime(output[i - 1][1], output[i][0]) == 1) {
            if (compareTime(output[i - 1][1], output[i][1]) == -1)
...
let calcFreeSlots = (c1, c2) => {
    let output = [];
    let flatC = flattenCalendar(c1, c2);
...
console.log(o3);
//# sourceMappingURL=Calenders.js.map